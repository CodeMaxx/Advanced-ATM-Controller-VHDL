...
int main(int argc, char *argv[]) {
	...
	// Added for CS254 Assignment

	struct arg_lit *assignOpt= arg_lit0("y", "custom", "			assignment");
	...
	if ( helpOpt->count > 0 ) {
		...
		printf("******* Modified for CS254 Lab projects *******\n");
	...
	if(assignOpt->count > 0) {
		...
		if ( isCommCapable ) {
			...
			if ( isRunning ) {
				for(int i =0;i<128;i++){
					uint8 buffer[1];
					// buffer[0] = i;
					flReadChannel(handle,(uint8) i, 1, buffer, &error);
					printf("%i\n", buffer[0]);
					buffer[0] += i;
					flWriteChannel(handle,(uint8) (i+1)%128, 1, buffer, &error);
					// printf("%i\n", buffer[0]);
					// pStatus = parseLine(handle, "r0 3", &error);
					// CHECK_STATUS(pStatus, pStatus, cleanup);
				}
			} else {
				fprintf(stderr, "The FPGALink device at %s is not ready to talk - did you forget --program?\n", vp);
		...
		// printf("Starting cs254 Assignment\n");
